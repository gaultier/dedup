...
struct thread_pool {
    ...
    _Atomic usize work_items_count;
...
static thread_pool_work_item* thread_pool_pop(thread_pool* pool) {
    ...
    pg_assert_int64(
        pool->work_item_i, <,
        (isize)__c11_atomic_load(&pool->work_items_count, __ATOMIC_ACQUIRE));
    thread_pool_work_item* w =
        pool->work_item_i < (isize)(__c11_atomic_load(&pool->work_items_count,
                                                      __ATOMIC_ACQUIRE) -
                                    1)
...
static void thread_pool_finish(thread_pool* pool) {
    __c11_atomic_fetch_add(&pool->end, 1, __ATOMIC_ACQUIRE);
...
static void thread_pool_push(thread_pool* pool, thread_pool_work_item w) {
    pg_assert_uint64(
        __c11_atomic_load(&pool->work_items_count, __ATOMIC_ACQUIRE), <=,
        pool->work_items_capacity);

    memcpy(&pool->work_items[__c11_atomic_load(&pool->work_items_count,
                                               __ATOMIC_ACQUIRE)],
           &w, sizeof(w));

    __c11_atomic_fetch_add(&pool->work_items_count, 1, __ATOMIC_ACQUIRE);
}
...
static void* thread_pool_worker(void* args) {
    ...
    while (true) {
        ...
        if (__c11_atomic_load(&pool->end, __ATOMIC_ACQUIRE)) return NULL;
    }
}
